...
class COMetaModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        # 这个方法需要你自己在子类实现返回metrics，下面是示例写法
        # metrics = {"test/loss": some_tensor}
        # self._test_outputs.append(metrics)
        # return metrics
        raise NotImplementedError("Please implement test_step in your subclass.")

    def on_test_epoch_end(self):
        # 汇总所有测试步骤的输出
        unmerged_metrics = {}
        for metrics in self._test_outputs:
        ...
        merged_metrics = {k: float(np.mean(v)) for k, v in unmerged_metrics.items()}
    ...
    def test_dataloader(self):
        ...
        test_dataloader = GraphDataLoader(
          self.test_dataset, batch_size=batch_size, shuffle=False, num_workers=self.args.num_workers
        )
    ...
    def val_dataloader(self):
        ...
        val_dataloader = GraphDataLoader(
          val_dataset, batch_size=batch_size, shuffle=False, num_workers=self.args.num_workers
        )
