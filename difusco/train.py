...
def main(args):
    ...
    # ✅ 提前创建日志/模型目录，避免 Missing logger folder
    os.makedirs(os.path.join(args.storage_path, "logs", project_name), exist_ok=True)
    os.makedirs(os.path.join(args.storage_path, "checkpoints"), exist_ok=True)
    ...
    tb_logger = TensorBoardLogger(
        save_dir=os.path.join(args.storage_path, "logs"),
        name=project_name
    )
    rank_zero_info(f"Logging to {tb_logger.log_dir}")

    checkpoint_callback = ModelCheckpoint(
        monitor='val/solved_cost', mode=saving_mode,
        save_top_k=3, save_last=True,
        dirpath=os.path.join(args.storage_path, "checkpoints"),
    )
    ...
    trainer = Trainer(
        ...
        use_distributed_sampler=False  # ✅ 解决 DistributedSampler 重复采样警告
    )
